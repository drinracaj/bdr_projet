...
const std::string Tenant::sql_req_sel_all =
    "SELECT * FROM " + ModelRegistry::model(typeid(Tenant));

const std::string Tenant::sql_req_sel_all_as_str =
    "SELECT id, CONCAT(firstname, ' ', lastname) AS as_str FROM " +
    ModelRegistry::model(typeid(Tenant));