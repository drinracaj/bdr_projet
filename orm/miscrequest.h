 ...
 public:
  ...
  static std::optional<std::vector<std::unique_ptr<Fees>>> get_all_fees(
    ...
    std::string req =
        ...
        "        EXTRACT(MONTH FROM o.start)::int as current_month,\n"
        "        EXTRACT(YEAR FROM o.start)::int as current_year,\n"
        "        (\n"
        ...
        "        occupancy_id,\n"
        "        CASE \n"
        "            WHEN current_month = 12 THEN 1\n"
        "            ELSE current_month + 1\n"
        "        END,\n"
        "        CASE \n"
        "            WHEN current_month = 12 THEN current_year + 1\n"
        "            ELSE current_year\n"
        "        END,\n"
        "        totVolume\n"
        "    FROM month_sequence\n"
        ...
        "            AND i.building_id = 1\n"
        "            AND i.date BETWEEN '2023-07-01' AND '2024-06-30'\n"
        "        ), 0) as total_invoice,\n"
        "        (\n"
